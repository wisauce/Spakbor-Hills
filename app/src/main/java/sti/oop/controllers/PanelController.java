...
public class PanelController {
    ...
    public void initialize() {
        ...
        try {
            ...
            // Add a Label for prompts/feedback in your FishingPanel.fxml with fx:id="fishingPromptLabel"
    ...
    // --- NPC Interaction Panel (Example from before, ensure it uses hideAllBottomPanels) ---
     ...
     * @param resultCallback Callback with OptionalInt.empty() if failed/cancelled, or OptionalInt.of(guessedNumber) on success.
     */
    public void showFishingGuessingGame(String prompt, int dynamicTarget, int minRange, int maxRange, int attempts, Consumer<OptionalInt> resultCallback) {
        if (fishingRootNode == null || fishingNumberInput == null || fishingConfirmButton == null || fishingPromptLabel == null) {
        ...
        UnaryOperator<TextFormatter.Change> filter = change -> {
            ...
            // This regex is a bit simplistic for arbitrary ranges, especially with leading zeros.
    ...
    private void handleFishingGuess() {
        ...
        if (guessedValue < fishingMinRange || guessedValue > fishingMaxRange) {
            updateFishingPrompt("Number must be between " + fishingMinRange + " and " + fishingMaxRange + ". Attempts left: " + (maxAttempts - currentAttempts));
            fishingNumberInput.clear();
            return;
        }
